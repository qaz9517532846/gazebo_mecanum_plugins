#include <gazebo_mecanum_plugins/gazebo_ros_mecanum_diff_drive.h>
...
namespace gazebo
{
    ...
    GazeboRosMecanumDiffDrive::GazeboRosMecanumDiffDrive()
    {
    ...
    void GazeboRosMecanumDiffDrive::Load(physics::ModelPtr parent, sdf::ElementPtr sdf)
    {
        ...
        callback_queue_thread_ = boost::thread(boost::bind(&GazeboRosMecanumDiffDrive::QueueThread, this));

        // listen to the update event (broadcast every simulation iteration)
        update_connection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboRosMecanumDiffDrive::UpdateChild, this));
    }

    void GazeboRosMecanumDiffDrive::publishWheelJointState()
    {
    ...
    void GazeboRosMecanumDiffDrive::publishWheelTF()
    {
        ros::Time current_time = ros::Time::now();
        for(int i = 0; i < 4; i++)
        {
    ...
    void GazeboRosMecanumDiffDrive::FiniChild()
    {
        alive_ = false;
        queue_.clear();
        queue_.disable();
    ...
    void GazeboRosMecanumDiffDrive::cmdVelCallback(const geometry_msgs::Twist::ConstPtr& cmd_msg)
    {
        boost::mutex::scoped_lock scoped_lock(lock);
        x_ = cmd_msg->linear.x;
        y_ = cmd_msg->linear.y;
    ...
    void GazeboRosMecanumDiffDrive::getWheelVelocity()
    {
    ...
    void GazeboRosMecanumDiffDrive::publishOdometry(double step_time)
    {
        ros::Time current_time = ros::Time::now();
    ...
    void GazeboRosMecanumDiffDrive::calkinematics(linear_vel &line_vel)
    {
