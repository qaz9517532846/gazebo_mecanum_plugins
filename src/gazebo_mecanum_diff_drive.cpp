#include <gazebo_mecanum_plugins/gazebo_mecanum_diff_drive.h>

namespace gazebo_mecanum_plugins
{
  GazeboMecanumRosDiffDrive::GazeboMecanumRosDiffDrive() : impl_(std::make_unique<GazeboMecanumRosDiffDrivePrivate>())
  {

  }
  
  GazeboMecanumRosDiffDrive::~GazeboMecanumRosDiffDrive()
  {

  }
  
  void GazeboMecanumRosDiffDrive::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    impl_->model_ = _model;
    ...
    impl_->cmd_vel_sub_ = impl_->ros_node_->create_subscription<geometry_msgs::msg::Twist>(impl_->cmd_topic_, qos.get_subscription_qos(impl_->cmd_topic_, rclcpp::QoS(1)),
                                                                                           std::bind(&GazeboMecanumRosDiffDrivePrivate::OnCmdVel, impl_.get(), std::placeholders::_1));
      ...
      for (index = 0; index < impl_->num_wheel_pairs_; ++index)
      {
        if(impl_->publish_wheel_tf_)
        {
          RCLCPP_INFO(impl_->ros_node_->get_logger(), "Publishing wheel transforms between [%s], [%s] , [%s] , [%s], [%s]",
                                                       impl_->robot_base_frame_.c_str(),
                                                       impl_->joints_[2 * index + GazeboMecanumRosDiffDrivePrivate::LEFT_REAR]->GetName().c_str(),
                                                       impl_->joints_[2 * index + GazeboMecanumRosDiffDrivePrivate::LEFT_FRONT]->GetName().c_str(),
                                                       impl_->joints_[2 * index + GazeboMecanumRosDiffDrivePrivate::RIGHT_FRONT]->GetName().c_str(),
                                                       impl_->joints_[2 * index + GazeboMecanumRosDiffDrivePrivate::RIGHT_REAR]->GetName().c_str());
        }
      }
    }
    ...
    impl_->update_connection_ = gazebo::event::Events::ConnectWorldUpdateBegin(std::bind(&GazeboMecanumRosDiffDrivePrivate::OnUpdate, impl_.get(), std::placeholders::_1));
  }
  
  void GazeboMecanumRosDiffDrive::Reset()
  {
    impl_->last_update_time_ = impl_->joints_[GazeboMecanumRosDiffDrivePrivate::LEFT_REAR]->GetWorld()->SimTime();
    for(unsigned int i = 0; i < impl_->num_wheel_pairs_; ++i)
    {
      if (impl_->joints_[2 * i + GazeboMecanumRosDiffDrivePrivate::LEFT_REAR] && impl_->joints_[2 * i + GazeboMecanumRosDiffDrivePrivate::LEFT_FRONT] &&
          impl_->joints_[2 * i + GazeboMecanumRosDiffDrivePrivate::RIGHT_FRONT] && impl_->joints_[2 * i + GazeboMecanumRosDiffDrivePrivate::RIGHT_REAR])
      {
        impl_->joints_[2 * i + GazeboMecanumRosDiffDrivePrivate::LEFT_REAR]->SetParam("fmax", 0, impl_->max_wheel_torque_);
        impl_->joints_[2 * i + GazeboMecanumRosDiffDrivePrivate::LEFT_FRONT]->SetParam("fmax", 0, impl_->max_wheel_torque_);
        impl_->joints_[2 * i + GazeboMecanumRosDiffDrivePrivate::RIGHT_FRONT]->SetParam("fmax", 0, impl_->max_wheel_torque_);
        impl_->joints_[2 * i + GazeboMecanumRosDiffDrivePrivate::RIGHT_REAR]->SetParam("fmax", 0, impl_->max_wheel_torque_);
      }
    }
  ...
  void GazeboMecanumRosDiffDrivePrivate::OnUpdate(const gazebo::common::UpdateInfo & _info)
  {
  ...
  void GazeboMecanumRosDiffDrivePrivate::UpdateWheelVelocities()
  {
  ...
  void GazeboMecanumRosDiffDrivePrivate::OnCmdVel(const geometry_msgs::msg::Twist::SharedPtr _msg)
  {
    std::lock_guard<std::mutex> scoped_lock(lock_);
  ...
  void GazeboMecanumRosDiffDrivePrivate::PublishOdometryTf(const gazebo::common::Time & _current_time)
  {
  ...
  void GazeboMecanumRosDiffDrivePrivate::PublishWheelsTf(const gazebo::common::Time & _current_time)
  {
  ...
  void GazeboMecanumRosDiffDrivePrivate::PublishOdometryMsg(const gazebo::common::Time & _current_time)
  {
  ...
  void GazeboMecanumRosDiffDrivePrivate::calkinematics(linear_vel &line_vel)
  {
  ...
  GZ_REGISTER_MODEL_PLUGIN(GazeboMecanumRosDiffDrive)
